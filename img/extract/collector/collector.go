...
func NewCollector(names []string, logger *log.Logger) (*Collector, error) {
	cfg := &packages.Config{
	...
	for _, pkg := range c.pkgs {
		for _, f := range pkg.Syntax {
			...
			ast.Inspect(f, func(n ast.Node) bool {
				...
				candidate, ok := c.start2node[loc.FullStart()]
				if ok {
					source = &candidate
				}
				...
				if source.DisplayName == "main.main" && obj.Pkg() != nil {
					...
					if tgtpkg != nil {
						...
						if tgtnode, ok := c.offset2node[tgtloc.FullOffset()]; ok {
							fmt.Printf("%s references %s in %s\n", source.DisplayName, tgtnode.DisplayName, tgtfile.Name())
						}
					}
				}
				return true
